...
class DualEncoder(torch.nn.Module):
    def __init__(self,
        ...
        if sbert_or_luar == "sbert":
            ...
            self.model_i = AutoModel.from_pretrained("rrivera1849/LUAR-MUD", trust_remote_code=True)
            self.model_j = AutoModel.from_pretrained("rrivera1849/LUAR-MUD", trust_remote_code=True)
    ...
    def forward(self, xi, xj):
        if self.sbert_or_luar == "sbert":
            ...
            input_ids_i = self.tokenizer(xi, return_tensors="pt", padding=True, truncation=True).to(self.model_i.device)
            input_ids_i['input_ids'] = input_ids_i['input_ids'].unsqueeze(1)
            input_ids_i['attention_mask'] = input_ids_i['attention_mask'].unsqueeze(1)

            embeddings_i = self.model_i(**input_ids_i)

            input_ids_j = self.tokenizer(xj, return_tensors="pt", padding=True, truncation=True).to(self.model_j.device)
            input_ids_j['input_ids'] = input_ids_j['input_ids'].unsqueeze(1)
            input_ids_j['attention_mask'] = input_ids_j['attention_mask'].unsqueeze(1)
            ...
            input_ids_i = self.tokenizer(xi, return_tensors="pt", padding=True, truncation=True).to(self.model_i.device)
            input_ids_j = self.tokenizer(xj, return_tensors="pt", padding=True, truncation=True).to(self.model_i.device)
            embeddings_i = self.model_i(**input_ids_i)
            embeddings_j = self.model_j(**input_ids_j)"""
